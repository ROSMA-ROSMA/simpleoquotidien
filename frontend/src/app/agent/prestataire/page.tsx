    ...
    return (
        <AgentLayout title="Prestataires" subtitle={loading ? 'Chargement…' : `${providers.length} prestataire${providers.length !== 1 ? 's' : ''} enregistré${providers.length !== 1 ? 's' : ''}`}>
            <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3.5 py-2.5 mb-5 max-w-md">
                ...
                <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Rechercher un prestataire ou une ville…" className="flex-1 outline-none text-[11px] bg-transparent" />
            </div>

            {loading ? (
                <p className="text-slate-400 text-[11px] text-center py-10">Chargement des prestataires…</p>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div key={p.id} className="bg-white rounded-2xl p-5 shadow-card border border-slate-100">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-[11px] text-slate-400 flex items-center gap-1"><IconMapPin className="w-3 h-3" /> {p.intervention_city ?? 'Ville non renseignée'}</p>
                                </div>
                            </div>
                    ...
                    {filtered.length === 0 && (
                        <p className="text-slate-400 text-[11px] col-span-full text-center py-10">Aucun prestataire ne correspond à cette recherche.</p>
                    )}
                </div>
            )}
        </AgentLayout>
