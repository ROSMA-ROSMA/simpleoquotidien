        ...
        <AgentLayout title="Recherche de prestataires" subtitle="Filtrez par ville d'intervention et catégorie de service">
            <div className="flex flex-wrap gap-3 mb-6">
                <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3.5 py-2.5 flex-1 min-w-[220px]">
                    ...
                    <input value={city} onChange={(e) => setCity(e.target.value)} placeholder="Ville d'intervention (ex : Ouagadougou)" className="flex-1 outline-none text-sm bg-transparent" />
                </div>
                <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)} className="px-3.5 py-2.5 rounded-xl border border-slate-200 bg-white text-sm font-semibold text-slate-600 outline-none focus:border-brand-teal">
                    ...
                    {results.map(p => (
                        <div key={p.id} className="bg-white rounded-2xl p-5 shadow-card border border-slate-100">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <div>
                                    <p className="font-bold text-brand-dark text-sm">{p.first_name} {p.last_name}</p>
                            ...
                            <Link href="/agent/comparaison" className="mt-3 flex items-center justify-center gap-2 w-full py-2 rounded-xl border border-slate-200 text-sm font-bold text-slate-600 hover:border-brand-teal hover:text-brand-teal transition-colors">
                                <IconMessage className="w-3.5 h-3.5" /> Comparer les prestataires
                            </Link>
                        </div>
                    ))}
                    {results.length === 0 && (
                        <p className="text-slate-400 text-sm col-span-full text-center py-10">Aucun prestataire ne correspond à ces critères.</p>
                    )}
                </div>
            )}
        </AgentLayout>
