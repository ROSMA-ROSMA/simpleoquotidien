...
import {
    IconMapPin, IconCalendar, IconUser, IconMail, IconMessage, IconArrowLeft,
    IconRefresh, IconSend, IconUserPlus, IconUsers,
...
function assignmentStatusBadge(status?: 'EN_ATTENTE' | 'ACCEPTEE' | 'REFUSEE') {
    switch (status) {
...
export default function AgentCommandeDetailPage({ params }: Props) {
    ...
    const loadBooking = useCallback(async () => {
        try {
            const res = await orderService.getById(bookingId);
            setBooking(res.data);
    ...
    const canQuote = [BookingStatus.ASSIGNED, BookingStatus.CONFIRMED].includes(booking.status);
    ...
    const handleSendContact = async () => {
        ...
        try {
            await orderService.contactClient(booking.id, contactMessage);
            setContactSent(true);
    ...
    const handleQuote = async () => {
        try {
            await orderService.sendQuote(booking.id, Number(quoteAmount), quoteNote || undefined);
    ...
    const handleClose = async () => {
        try {
            await orderService.close(booking.id);
            loadBooking();
        } catch (e) { alert(e instanceof Error ? e.message : 'Erreur'); }
    };
    ...
    return (
        <AgentLayout title={`Commande #SQ-${booking.id}`} subtitle="Détails et actions">
            <div className="flex items-center gap-3 mb-5 flex-wrap">
                <Link href="/agent/commandes" className="inline-flex items-center gap-2 text-slate-500 hover:text-brand-teal font-medium text-[11px] transition-colors">
                ...
                {booking.created_at && <span className="text-[11px] text-slate-400">Créée le {formatDateTime(booking.created_at)}</span>}
            </div>

            <div className="flex items-start justify-between gap-4 flex-wrap mb-6">
                <div>
                    <h1 className="text-xl font-extrabold text-brand-dark">Commande #SQ-{booking.id}</h1>
                    <p className="text-[11px] text-slate-500 mt-1">{getBookingTitle(booking)}</p>
                </div>
                <div className="flex gap-3 flex-wrap">
                    ...
                    {canAssign && (
                        <Link href={`/agent/commandes/${booking.id}/assignation`}>
            ...
            {showContact && (
                <div className="bg-white rounded-2xl p-6 shadow-card border border-slate-100 mb-6">
                    ...
                    <textarea
                        ...
                        rows={5}
                        className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white text-[11px] mb-3 outline-none focus:border-brand-teal resize-none"
                    />
                    <div className="flex items-center gap-3">
                        ...
                        {contactSent && <span className="text-[11px] font-semibold text-emerald-600">Message envoyé au client.</span>}
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-6">
                    <div className="bg-white rounded-2xl p-6 shadow-card border border-slate-100">
                        ...
                        <div className="flex items-center gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <p className="text-[11px] text-slate-400">{booking.client?.email ?? '—'}</p>
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-card border border-slate-100">
                        ...
                        {booking.message && <p className="text-[11px] text-slate-500 italic mt-4 pt-4 border-t border-slate-100">&ldquo;{booking.message}&rdquo;</p>}
                    </div>

                    {canQuote && (
                        <div className="bg-white rounded-2xl p-6 shadow-card border border-slate-100">
                            <h3 className="font-bold text-brand-dark mb-3 flex items-center gap-2"><IconSend className="w-4 h-4 text-brand-teal" /> Envoyer un devis</h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                                <input type="number" value={quoteAmount} onChange={(e) => setQuoteAmount(e.target.value)} placeholder="Montant (FCFA)" className="px-4 py-3 rounded-xl border border-slate-200 text-[11px] outline-none focus:border-brand-teal" />
                                <input value={quoteNote} onChange={(e) => setQuoteNote(e.target.value)} placeholder="Note (optionnel)" className="px-4 py-3 rounded-xl border border-slate-200 text-[11px] outline-none focus:border-brand-teal" />
                            </div>
                            <Button disabled={!quoteAmount} onClick={handleQuote}>Envoyer le devis au client</Button>
                        </div>
                    )}
                ...
                <div className="space-y-6">
                    <div className="bg-white rounded-2xl p-6 shadow-card border border-slate-100">
                        ...
                        <p className="text-[11px] text-slate-600">{booking.address}</p>
                        <p className="text-[11px] text-slate-400 mt-1">{booking.intervention_city ?? 'Ville non renseignée'}</p>
                    </div>
                        ...
                        {assignedProvider ? (
                            <div>
                                <div className="flex items-center gap-3 mb-3">
                                    ...
                                    <div>
                                        <p className="font-bold text-brand-dark text-[11px]">{assignedProvider.first_name} {assignedProvider.last_name}</p>
                                        <p className="text-[11px] text-slate-400">{assignedProvider.intervention_city ?? 'Ville non renseignée'}</p>
                                    </div>
                                </div>
                            ...
                            <div className="text-center py-6 text-slate-400">
                                ...
                                <p className="font-semibold text-slate-600 text-[11px]">Aucun prestataire assigné</p>
                                <p className="text-[11px] mt-1">Utilisez la recherche pour trouver le meilleur profil.</p>
                                {canAssign && (
                                    <Link href={`/agent/commandes/${booking.id}/assignation`}>
                    ...
                    {canReassign && (
                        <div className="bg-white rounded-2xl p-6 shadow-card border border-slate-100">
                            ...
                            <p className="text-[11px] text-slate-400 mb-3">Le prestataire précédent a refusé cette commande — assignez-en un nouveau.</p>
                            <Link href={`/agent/commandes/${booking.id}/assignation`}>
