...
import { BookingStatus, UserRole, BOOKING_STATUS_LABELS, PaymentStatus, User, Category } from '@/types';
import { IconUsers, IconBriefcase, IconCalendar, IconCurrencyDollar, IconPlus, IconEdit, IconCircleCheck, IconStar, IconEye, IconTrash, IconCheck, IconX, IconFileText, IconPhoto } from '@tabler/icons-react';

/* ──── Validation Sub-Component ──── */
function ValidationSection() {
    type PendingProvider = Awaited<ReturnType<typeof providerService.getPending>>['data'][number];
    ...
    return (
        <div className="animate-in">
            <h1 className="text-3xl font-extrabold text-brand-dark mb-6">Validations en attente</h1>
            {loading ? (
                ...
                <div className="bg-white rounded-2xl p-10 text-center shadow-card border border-slate-100">
                    <p className="text-slate-400 text-lg">Aucune validation en attente</p>
                </div>
            ) : (
...
export default function AdminDashboardPage() {
    ...
    const pendingProviders = users.filter(u => u.role === UserRole.PROVIDER && !u.is_verified);
    ...
    return (
        <div className="flex min-h-screen bg-brand-surface">
            ...
            <main className="flex-1 ml-64 p-8">
                ...
                {activeSection === 'dashboard' && (
                    <div className="animate-in">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                            ...
                            <StatCard label="Services Actifs" value={services.length} icon={<IconBriefcase className="w-6 h-6" />} color="teal" />
                                        ...
                                        {users.map(u => {
                                            ...
                                            return (
                                                <tr key={u.id} className="border-t border-slate-50 hover:bg-slate-50/50">
                                                    ...
                                                    <td className="py-3 px-4">{u.is_active ? <Badge variant="success">Actif</Badge> : <Badge variant="danger">Inactif</Badge>}</td>
                ...
                {/* Services — pas de backend, reste sur le store */}
                {activeSection === 'services' && (
                    <div className="animate-in">
                        <div className="flex items-center justify-between mb-6">
                            <h1 className="text-3xl font-extrabold text-brand-dark">Services Actifs</h1>
                            <Link href="/admin/services/create"><Button size="sm" icon={<IconPlus className="w-4 h-4" />}>Ajouter</Button></Link>
                        </div>
                        <div className="bg-white rounded-2xl shadow-card border border-slate-100 overflow-hidden">
                            <table className="w-full text-base">
                                ...
                                <tbody>
                                    {services.map(s => (
                                        <tr key={s.id} className="border-t border-slate-50 hover:bg-slate-50/50">
                                            ...
                                            <td className="py-3 px-4 text-slate-500">{s.category?.name}</td>
                                            <td className="py-3 px-4 text-slate-500">{s.provider?.first_name}</td>
                                            ...
                                            <td className="py-3 px-4 flex gap-2">
                                                ...
                                                <Link href={`/admin/services/edit/${s.id}`}><Button variant="ghost" size="sm"><IconEdit className="w-4 h-4" /></Button></Link>
                                                <Button variant="ghost" size="sm" onClick={() => deleteService(s.id)}><IconTrash className="w-4 h-4 text-red-400" /></Button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
