...
from .models import Utilisateur, PrestataireProfile, Assignment, Subscription, Notes, Notification, RoleChoices
...
class PrestataireCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Désactiver le compte jusqu'à validation par l'admin
        user.is_active = False
        user.save(update_fields=['is_active'])

        # Création du profil Prestataire associé (verification_status=EN_ATTENTE par défaut)
...
class AssignmentSerializer(serializers.ModelSerializer):
    class Meta:
        model = Assignment
        fields = ['id', 'methode', 'status', 'reason', 'order', 'prestataire', 'qualifier', 'date_creation']
        read_only_fields = ['id', 'status', 'reason', 'date_creation']


class NotificationSerializer(serializers.ModelSerializer):
    class Meta:
        model = Notification
        fields = ['id', 'user', 'type', 'title', 'message', 'order', 'is_read', 'date_creation']
        read_only_fields = ['id', 'user', 'type', 'title', 'message', 'order', 'date_creation']
